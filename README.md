...
- **CALENDAR 패널** — 맥 캘린더를 EventKit으로 직접 읽고 씀 (iCloud·구글 등 시스템 설정 > 인터넷 계정에 추가된 모든 캘린더 포함 — 개발자 API가 막힌 학교 구글 계정도 인터넷 계정으로 추가만 하면 잡힘). 14일 일정을 날짜별로, NEXT 카운트다운, 빠른 추가(시간 비우면 종일), 삭제. 권한: 시스템 설정 > 개인정보 보호 및 보안 > 캘린더
