...
- **상시 대기(ALWAYS) — "내가 옴니에게 말할 때만" 3단 게이트** (v0.65.0)
  ...
  - **2단 내 목소리**: 화자 임베딩(Resemblyzer GE2E) 집합 최대 유사도. **VOICE ID 저장소** `~/.omni/voice/profiles.json` — 이름 붙인 ID를 여러 개 두고 골라 쓰며(NEW ID / 선택), **ENROLL ME**는 선택한 ID에 15초 발화를 2초 창으로 쪼개 **덧붙여 누적**(최대 40개, 톤·거리를 바꿔가며 반복할수록 정확). **OTHER**는 TV·가족·친구 등 **타인 목소리를 부정 예시로 등록** — 판정 때 내 집합과 타인 집합 유사도를 함께 봐서 타인 쪽에 더 가까우면 내 유사도가 높아도 거부(프로필이 넓어져 비슷한 목소리를 삼키는 것을 방지), 적응 학습에서도 타인에 더 가까운 발화는 제외. **ANALYZE**는 자기 유사도(leave-one-out 평균/최저)·타인별 최대 유사도·구분 여유·자동 임계(타인 있으면 중간점, 없으면 자기유사도−0.15)·판정(명확/보통/약함)을 리포트하고 여유가 좁으면 경고. 호출어로 확정된 발화로 프로필 자동 적응
