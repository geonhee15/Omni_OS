...
- **상시 대기(ALWAYS) — "내가 옴니에게 말할 때만" 3단 게이트** (v0.65.0)
  - ALWAYS 버튼 한 번으로 앱이 켜져 있는 동안 계속 듣되, 유튜브·TV·타인의 대화·혼잣말·다른 사람과의 대화에는 절대 답하지 않음
  - **1단 사람 말**: 사이드카 `scripts/omni_gate.py`(voice_engine venv)가 마이크를 직접 열고 Silero VAD로 발화 구간만 자름
  - **2단 내 목소리**: 화자 임베딩(Resemblyzer GE2E) 코사인 유사도 — VOICE ID > ENROLL로 15초 등록(`~/.omni/voice_profile.json`, 자기 유사도−0.15 자동 임계 0.70~0.78). 다른 사람·TTS·TV 목소리는 여기서 탈락 (측정: 본인 0.83~0.93 vs 타인 0.40~0.75). 호출어로 확정된 발화로 프로필이 계속 적응
  - **3단 옴니에게 한 말**: 통과한 발화의 전사(gpt-4o-transcribe, "옴니" 편향 프롬프트)에 호출어 **"옴니(야)"** 가 있으면 응답. 옴니가 답을 마친 뒤 8초 안의 발화는 Haiku 분류기가 "이어지는 대화"인지 판정(다른 사람에게 한 말이면 NO). 그 외는 무시하고 **모델 문맥에서도 삭제**(conversation.item.delete) — 흘려들은 소리가 나중 답에 섞이지 않음
  - 옴니가 말하는 동안은 마이크 무시(에코 차단). 리얼타임 세션은 수동 턴(서버 VAD 없음), 사이드카가 append+commit
  - 판정 사유가 OMNI_AI 로그(흐리게)와 `~/.omni/gate.log`에 남음. 앱 재시작 시 자동 복귀. 검증: 스피커→마이크 E2E 4라운드(타인 호출어 거부, 혼잣말 무시, 호출어 응답, 이어지는 질문 응답, 창 안 잡담 거부, 유사 음색 거부)
