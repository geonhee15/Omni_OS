...
- **CAMERA 패널** (v0.78) — 집 카메라(Tapo C210 등)를 클라우드 없이 맥에서 직접. 사이드카 `scripts/omni_camera.py`(halo/venv)가 `http://127.0.0.1:8484`에서 RTSP→MJPEG(ffmpeg 8fps 640px) 라이브·스냅샷·비전 설명(`/describe`, Haiku)·검색(같은 /24의 RTSP 554+ONVIF 2020, DEEP는 Tapo 계정으로 모델명)·사람 감지(MediaPipe EfficientDet-Lite0, 0.7초마다, 2연속 감지 시 사건 기록 + 스냅샷 `~/.omni/camera/` + 기억 observe, 카메라당 1분 쿨다운)를 제공. 회전(PTZ)·프라이버시(렌즈 가림)는 python-kasa 카메라 모듈(`omni_smart.py camera`)로 Tapo 클라우드 계정 경유. **설정**: Tapo 앱에서 카메라 추가 → 설정 > 고급 설정 > **카메라 계정** 생성 → 패널 SETUP에 저장 → DISCOVER. 계정은 `~/.omni/camera.json`(0600). 패널: 카드마다 LIVE/SNAPSHOT/LOOK/WATCH/PRIVACY/PTZ 화살표, 사건 목록(썸네일). 옴니 전권: `camera_control` 도구(텍스트·LIVE·안경 — "방 카메라 봐줘", "누구 있어?", "카메라 가려줘", "왼쪽으로 돌려"), `cam.look/status/snapshot/privacy/ptz/watch/discover/start/stop` 액션. 새 사람 감지 사건은 OMNI_AI 로그에 뜨고 LIVE 중이면 옴니가 말로 알린다(1분에 한 번, 학교 모드면 조용)
- **HALO GLASSES 패널** (v0.77) — 안경용 옴니의 작업 공간. 앞으로 안경 관련 기능은 여기에 모인다.
  - **폰 안경(Phone Glasses)**: 안경이 오기 전에 폰을 안경처럼 쓴다. START PHONE GLASSES → `halo/phone_glasses.py`가 `https://<맥IP>:8443`(로컬 CA 자체 서명, 폰에 1회 설치)으로 뜨고, 패널에 주소·QR·접속 수·상태·마지막 자막·도구 목록·로그가 보인다. 폰 브라우저에서 START를 탭하면 **폰 뒤 카메라 = 안경 눈(배경 미리보기 + `look_camera` "지금 뭐 보여?")**, **폰 마이크 = 안경 귀(앱 ALWAYS와 같은 음성 게이트 사이드카: 내 목소리·호출어·이어가기)**, **폰 화면 = 안경 HUD(256×256 원형 16색, `hud.py` 스프라이트를 `hud_compose.py`가 합성한 PNG를 WebSocket으로)**, **폰 스피커 = 안경 스피커(marin 24k)**, 화면 탭 = 알림 브리핑. 학교 모드면 듣지 않는다. 옴니 액션 `halo.start/stop/status`, 안경 `app_action` 스펙에도 포함
  - **안경 옴니 = 앱 옴니** (`halo/glasses_core.py`, 에뮬레이터 `live_demo.py`와 폰 안경이 공유): ask_brain·get_time·카톡·지메일·계산기·날씨·뉴스·시세·**smart_control(Tapo 직접 제어)**·**quiet_mode**·**save_memory/recall_memory**·**look_camera**·**run_shell**·캘린더·add_event·app_action(패널·web.search·computer·ui.*·halo.*). 앱과 같은 `~/.omni/memory` 기억·두뇌·메일박스
- **SCHOOL MODE (학교·시험 모드)** (v0.76) — 학교에서 카메라 표시등이 켜져 있던 사고 이후 추가. 하단 바 **SCHOOL MODE** 버튼·`quiet_mode` 도구("학교 모드 켜줘", "카메라 다 꺼줘")·`quiet.on[:분]`/`quiet.off` 액션(안경 포함)·SP-1 메뉴 막대 중 어디서 켜도 **카메라 감시(SP-1)·마이크 상시 대기·LIVE·화면 관찰·제스처 명령이 한 번에 멈춘다**. 상태는 `~/.omni/store/quiet_mode.json`으로 두 앱이 공유. **자동**: SP-1이 20초마다 집 공유기(게이트웨이 MAC)를 확인해 `presence.json`에 쓰고, 집 밖(학교 와이파이·핫스팟·네트워크 없음)이면 옴니도 자동으로 학교 모드가 되어 상시 대기 자동 시작까지 막는다. 집으로 돌아오면 해제되지만 상시 대기는 사용자가 다시 켠다
