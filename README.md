...
- **OMNI_AI** — 한국어 음성 인터페이스 (1단계: 한국어 · 지능 · 레트로 로봇 보이스).
  ...
  **패널 제어**: "아크스캔 열어"처럼 말하면 응답의 `[[OPEN:키]]` 태그로 12개 패널 어디든
  ...
  에러로 세션을 끊으면 조용히 재시작해 청취를 유지. 비한국어 TTS는 kNN 치환 없이 네이티브 발음 위에 대사팩 톤(적응 피치 정합 · 완화된 LTAS 매칭 · 라이트 링모드)만 입혀 한국어 억양 문제를 차단. 심층 액션 `[[ACT:proj.editor:이름:도구]]`로 "아크스캔 3D 에디터 열어줘" 같은 패널 내부 체인 실행, 시각·시스템 현황은 스냅샷으로 즉답.
