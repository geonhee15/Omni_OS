...
CASES = [
    ...
    ("응 알겠어 고마워", F(USER, 3), "내일 미술 수업은 오후 2시 20분에 끝납니다.", "미술 수업 언제 끝나?", None),  # 종료 인사 — 답해도 안 해도 자연스러움
    ...
    # 옴니가 먼저 질문한 직후의 반문 — 혼자 있는데 '얘/너/네 앱'을 남에게 하는 말로 오판하던 케이스
    ("아 그러니까 네 앱에서 말하는 거야?", F(USER, 4), "지금 보고 있는 Command Bridge 대시보드는 어떤 용도로 만드는 시스템인가요?", "(없음)", True),
    ("얘 말하는 거야?", F(USER, 9), "지금 보고 있는 Command Bridge 대시보드는 어떤 용도로 만드는 시스템인가요?", "아 그러니까 네 앱에서 말하는 거야?", True),
    ("아니 니 앱에서 말하는 커맨드 브릿지 물어보는 거냐고", F(USER, 20), "지금 보고 있는 Command Bridge 대시보드는 어떤 용도로 만드는 시스템인가요?", "얘 말하는 거야?", True),
    ("너 말하는 거냐고", F(USER, 30), "네, 알겠습니다. 어떤 작업을 진행할지 지정해 주십시오.", "다른 사람한테 말한 게 아니라 너한테 말한 거 맞아", True),
]
